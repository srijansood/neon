...
def build_vgg():
    ...
    for nofm, i in zip([64, 128, 256, 512, 512], xrange(1, 6)):
        ...
        layers.append(Pooling(2, op="max", strides=2, name="pool{}".format(i)))
...
def total_loss(generated_image, content_names = ['conv4_2'],
    ...
    global total_derivative, content_derivative, style_derivative
    c_loss = s_loss = 0
    c_diff = []
    s_diff = []
    ...
    for layer in reversed(content_names):
        ...
        # print(layer + " done")
        c_diff.append(delta)
        content_derivative[:] = content_derivative + delta

    content_derivative[:] = c_contrib * content_derivative
    ...
    for layer in reversed(style_names):
        ...
        # print(layer + " done")
        s_diff.append(delta)
        style_derivative[:] = style_derivative + delta

    style_derivative[:] = s_contrib * style_derivative
...
def main():
    ...
    for i in xrange(1000):
        ...
        loss = total_loss(generated, style_names=['conv5_1'])
...
def allocate_buffers(min):
    global delta, total_length, gradient
    global total_derivative, content_derivative, style_derivative
    total_length = (min**2) * 3
    # Buffer allocations
    delta = be.zeros((total_length, 1))
    total_derivative = be.zeros((total_length, 1))
    content_derivative = be.zeros((total_length, 1))
    style_derivative = be.zeros((total_length, 1))
